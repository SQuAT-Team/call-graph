package io.github.squat_team.callgraph.export.graph;
 ...
 * </dl>
 *
 * @version v0.5.2 2018/05/06 (May) -- Patch of Sebastian Frank (Windows 10 fix
 *          + dependency solver integration)
...
public class GraphViz {
	...
	public GraphViz(GraphVizConfiguration config) {
		TEMP_DIR = config.getTempPath();
		DOT = config.getDotPath();
		
		log.info("Start GraphViz Pipeline");
		log.info("DOT Location: " + DOT);
		log.info("TEMP_DIR Location: " + TEMP_DIR);		
	}
	 ...
	 */
	public byte[] getGraph(String dot_source, String type, String dotFileName) {
		File dot;
		byte[] img_stream = null;

		try {
			dot = writeDotSourceToFile(dot_source, dotFileName);
			if (dot != null) {
	...
	private File writeDotSourceToFile(String str, String dotFileName) throws java.io.IOException {
		...
		try {
			...
			fout.write(str);
			BufferedWriter br = new BufferedWriter(new FileWriter(dotFileName+".dot"));
			br.write(str);
			br.flush();
			br.close();
			fout.close();
